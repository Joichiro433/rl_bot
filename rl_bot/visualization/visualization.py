...
class TradingGraph:
    ...
    def render(self, Date, Open, High, Low, Close, Volume, net_worth, trades):
        ...
        self.ax1.clear()
        candlestick_ohlc(self.ax1, self.render_data, width=0.8/24, colorup=params.UP_COLOR, colordown=params.DOWN_COLOR, alpha=0.8)
        # Put all dates to one list and fill ax2 sublot with volume
        Date_Render_range = [i[0] for i in self.render_data]
        self.ax2.clear()
        self.ax2.fill_between(Date_Render_range, self.Volume, 0, color=params.VOLUME_COLOR, alpha=0.8)
        # draw our net_worth graph on ax3 (shared with ax1) subplot
        self.ax3.clear()
        self.ax3.plot(Date_Render_range, self.net_worth, color=params.NETWORTH_COLOR)
        ...
        for trade in trades:
            ...
            if trade_date in Date_Render_range:
                if trade['type'] == 'buy':
                    ...
                    self.ax1.scatter(trade_date, high_low, c=params.UP_COLOR, label='blue', s = 120, edgecolors='none', marker="^")
                else:
                    high_low = trade['High']+10
                    self.ax1.scatter(trade_date, high_low, c=params.DOWN_COLOR, label='red', s = 120, edgecolors='none', marker="v")
        ...
        cv2.imwrite('../outputs/images/' + f'{self.conter:05}.jpg', img)
        self.conter += 1
